...
int main(int argc, char const *argv[])
{
  ...
  FindTheBest(mahasiswa, jumlah_mahasiswa);
...
void FindTheBest(Data *data, int total_data)
{
  int maks = 0, posisi;
  for (int i = 0; i < total_data; i++)
  {
    if (data[i].nilai_akhir > maks)
    {
      maks = data[i].nilai_akhir;
      posisi = i;
    }
  }
  printf("\n\nTotal mahasiswa = %d\n\n", total_data);
